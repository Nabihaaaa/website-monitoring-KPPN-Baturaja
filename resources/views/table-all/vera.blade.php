@extends('main')
@section('title')
    <a class="navbar-brand" href="{{url('table-kegiatan-all/seksi-vera')}}">
        Tabel Kegiatan
    </a>
@endsection
@section('sidebar')
            ...
            <div class="card">
                ...
                <div class="card-content">
                    <table id="bootstrap-table" class="table table-striped ">
                        <div class="toolbar" >
                            <a class="btn btn-primary" href="{{url('/table-kegiatan-all/vera/add')}}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($upload->where($f_key, $item->id)->first() == null)
                                            <form method="get" action="{{url('/table-kegiatan-all/vera/undangan/upload/'.$item->id)}}" enctype="multipart/form-data">
                                        ...
                                        @else
                                            <form method="get" action="{{url('/table-kegiatan-all/vera/undangan/index/'.$item->id)}}" enctype="multipart/form-data">
                                    ...
                                    <td>
                                        @if ($item->realisasi_pelaksanaan == null && $upload->where($f_key, $item->id)->first() != null)
                                            <form method="get" action="{{url('/table-kegiatan-all/vera/upload/'.$item->id)}}" enctype="multipart/form-data">
                                                <button class="btn btn-danger btn-fill btn-wd" type="submit">Upload Bukti</button>
                                            </form>
                                        @elseif($item->realisasi_pelaksanaan != null)
                                            <form method="get" action="{{url('/table-kegiatan-all/vera/bukti/'.$item->id)}}" enctype="multipart/form-data">
                                    ...
                                    <td>
                                      <div class="table-icons">
                                        <a rel="tooltip" title="Edit" class="btn btn-simple btn-warning btn-icon table-action edit" href="{{url('/table-kegiatan-all/vera/edit/tabel/'.$item->id)}}">
                                            <i class="ti-pencil-alt"></i>
                                        </a>
                                        <a rel="tooltip" title="Remove" class="btn btn-simple btn-danger btn-icon table-action remove" onclick="demo.deleteAll('warning-message-and-cancel', {{$item->id}},'vera')">
                                            <i class="ti-close"></i>
                                        </a>
                                    </div>
                                    </td>
